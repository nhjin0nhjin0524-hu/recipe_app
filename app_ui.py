...
st.markdown("""
    ...
    /* 기본 공통 디자인 */
    .stButton > button { border-radius: 12px; border: 1px solid #E2E8F0; background: white; color: #64748B; font-weight: 600; }
    ...
    /* 🚨 1. [메뉴바 전용] 5칸짜리 구역만 골라서 한 줄로 딱 고정! */
    ...
    div[data-testid="stHorizontalBlock"]:has(> div:nth-child(5):last-child) > div[data-testid="column"] {
        ...
        flex: 1 1 20% !important; /* 정확히 20%씩 배분 */
    }
    div[data-testid="stHorizontalBlock"]:has(> div:nth-child(5):last-child) .stButton > button p {
        white-space: nowrap !important; /* 줄바꿈 절대 방지 */
        font-weight: 900 !important; /* 아주 진하게 */
        margin: 0 !important;
    }

    /* 📱 모바일 환경 최적화 */
    @media (max-width: 768px) {
        /* 상단 타이틀 및 부제목 크기 대폭 축소 */
        .main-title { font-size: 24px !important; letter-spacing: -1px !important; margin-bottom: 5px !important; }
        .sub-title { font-size: 8px !important; margin-bottom: 15px !important; }
        h3, .stMarkdown h3 { font-size: 15px !important; margin-bottom: 8px !important; }

        /* 메뉴바 버튼 폰트 및 높이 축소 (3글자도 안 깨짐) */
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(5):last-child) .stButton > button { 
            height: 32px !important; padding: 0px !important; 
        }
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(5):last-child) .stButton > button p {
            font-size: 8.5px !important; letter-spacing: -0.5px !important; 
        }

        /* 🚨 2. [대시보드 지표 전용] 3칸짜리 구역 모바일 가로 고정 */
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(3):last-child):has(h4) {
            flex-wrap: nowrap !important;
        }
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(3):last-child):has(h4) > div[data-testid="column"] {
            min-width: 0px !important;
            flex: 1 1 33.3% !important;
        }
        .dash-card { padding: 8px 4px !important; margin-bottom: 5px !important; border-radius: 8px !important; }
        .dash-card h4 { font-size: 8px !important; margin-bottom: 0px !important; }
        .dash-card h2 { font-size: 15px !important; line-height: 1.1 !important; }

        /* 🚨 3. [냉장고 카드 전용] 4칸짜리를 모바일에서 넓게 2칸(50%)씩 2줄로 배치! */
